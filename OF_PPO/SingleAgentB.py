...
class Subtraction(SingleRLAgent): 
    def __init__(self, agent_params, number_of_digits = 8):
        super().__init__(agent_params)
        self.reward_done_function = substraction_reward
    ...
    def reset(self, specific = None, input_number = None): 
        ...
        if (specific is None):       
            self.addendum_2 = self.sampler()
            self.addendum_1 = np.random.randint(0,self.addendum_2 , size=1)[0]
            self.ext_repr.random_reset(self.addendum_2, self.params['ext_shape'][1])
            #self.ext_repr.reset()
            self.n_objects = self.addendum_2 - self.addendum_1
            self.fingerlayer.reset()  
        ...
        if (flag or agent.sign) and (agent.pos_number_reading == agent.signpost):
            ...
            if agent.pos_number_reading+1 == len(agent.addendum_1_list_of_digits) or agent.sign:
                ...
                if (not agent.sign) and (not agent.params['auto_signpost']):
                    ...
                    agent.addendum_2 = agent.sampler()
                    agent.addendum_1 = np.random.randint(0,agent.addendum_2 , size=1)[0]
                    agent.ext_repr.random_reset(agent.addendum_2, agent.params['ext_shape'][1])
...
class TestEnv_Both(Both_operation):
    ...
    def extract_with_fixed_len(self):
        if len(self.list_number) > 0:
            y = self.list_number.pop(0)
        else:
            y = 0
        return(y)
    ...
    def reset(self): 
        ...
        self.is_addition = self.list_operation.pop(0)
        ...
        if (flag or agent.sign) and (agent.pos_number_reading == agent.signpost):
            ...
            if agent.pos_number_reading+1 == len(agent.addendum_1_list_of_digits) or agent.sign:
                ...
                if (not agent.sign) and (not agent.params['auto_signpost']):
                    ...
                    if len(agent.list_operation) == 0:
                        done = True
                    else:
                        agent.is_addition = agent.list_operation.pop(0)
