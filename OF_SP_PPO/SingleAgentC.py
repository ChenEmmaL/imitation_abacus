...
class SingleRLAgent(gym.Env):
    ...
    def step(self, action):
        ...
        if self.quasi_done == True:
            self.done = True
        else:
            self.done = False
        
        reward, self.quasi_done, self.solved, partial_list_digit, j = self.reward_done_function(self)
        
        self.state = self.update_state()

        if(self.timestep > self.max_episode_length):
            if self.sign:
                self.info['tuple'] = (1, partial_list_digit, tuple(j), self.fingerlayer.pos_x, self.fingerlayer.pos_y, 1, self.signpost, self.pos_number_reading, 3, self.addendum_2, self.addendum_1)
            else:
                self.info['tuple'] = (0, partial_list_digit, tuple(j), self.fingerlayer.pos_x, self.fingerlayer.pos_y, 1, self.signpost, self.pos_number_reading, 3, self.addendum_2, self.addendum_1)
            
            self.quasi_done = True
...
class Both_operation(SingleRLAgent): 
    ...
    def reset(self): 
        ...
        self.info["solved"] = 0
        self.info["tuple"] = None
        self.quasi_done = False
        self.max_episode_length = 30
...
class TestEnv_Both(Both_operation):
    def __init__(self, agent_params, number_of_digits = None):
        super().__init__(agent_params)
...
def reward_pair_both(agent): #used in Test_env, for testing paired sums
    info = False
    done = False
    ...
    reward = agent.params['base_reward']
    if (agent.action == 'move_and_slide') or (agent.action == 'submit'):
        agent.max_episode_length = agent.timestep+100
        if(agent.action == 'move_and_slide'):
            flag = True
            for index in range(agent.fingerlayer.pos_y + 1):
                if len(partial_list_digit) <= index:
            ...
            if flag and (agent.signpost == agent.pos_number_reading):
                ...
                if agent.signpost != agent.pos_number_reading:
                    agent.info['tuple'] = (0, partial_list_digit, tuple(j), agent.fingerlayer.pos_x, agent.fingerlayer.pos_y, 1, agent.signpost, agent.pos_number_reading, 0, agent.addendum_2, agent.addendum_1)
                else:
                    agent.info['tuple'] = (0, partial_list_digit, tuple(j), agent.fingerlayer.pos_x, agent.fingerlayer.pos_y, 0, agent.signpost, agent.pos_number_reading, 0, agent.addendum_2, agent.addendum_1)
        elif (agent.action == 'submit'):
            flag = True
            for index, el in enumerate(partial_list_digit):
                if partial_list_digit[index] != j[index]:
            ...
            if (flag or agent.sign) and (agent.pos_number_reading == agent.signpost):
                ...
                reward -= 1
                if agent.sign:
                    agent.info['tuple'] = (1, partial_list_digit, tuple(j), agent.fingerlayer.pos_x, agent.fingerlayer.pos_y, 1, agent.signpost, agent.pos_number_reading, 1, agent.addendum_2, agent.addendum_1)
                else:
                    agent.info['tuple'] = (0, partial_list_digit, tuple(j), agent.fingerlayer.pos_x, agent.fingerlayer.pos_y, 1, agent.signpost, agent.pos_number_reading, 1, agent.addendum_2, agent.addendum_1)
                done = True
    return reward, done, info, partial_list_digit, j 